    ...
    changePassword(id, newpassword, password){
        ...
        return new Promise((resolve, reject) => {
            if(typeof password !== 'string' ||
                ...
                newpassword.trim() === "") return reject(["Password is required"]);
            password = password.trim();
            newpassword = newpassword.trim();
            self.get(id).catch(err => {
                console.error(err);
                return reject(["An Unknown issue occured, please contact an admin. 3468"]);
            }).then( user => {
                if(!user) return reject(["Invalid User/Password."]);
                crypto.pbkdf2(password, user.password.salt, iterations, 512, 'sha512', (err, hash) => {
                    if(err){
                        console.error(err);
                        return reject(["An Unknown issue occured, please contact an admin. 2401"]);
                    }
                    hash = hash.toString('hex');
                    if(hash !== user.password.hash) return reject(["Invalid User/Password."]);
                    // gen new hash
                    let salt = crypto.randomBytes(256).toString('hex');
                    crypto.pbkdf2(newpassword, salt, iterations, 512, 'sha512', (err, hash) => {
                        if(err){
                            console.error(err);
                            return reject(["An Unknown issue occured, please contact an admin. 6458"]);
                        }
                        ...
                        user.save((err)=>{
                            if(err){
                                console.error(err);
                                return reject(["An Unknown issue occured, please contact an admin. 8732"]);
                            }
                            resolve(user);
                        });
                    });
            ...
            self.get(id).catch(reject).then((user)=>{
                ...
                for(var i in user.tokens){
                    if(
                        ...
                        && !user.tokens[i].used 
                        && user.tokens[i].expires > new Date()
