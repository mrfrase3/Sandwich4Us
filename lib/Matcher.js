...
var requestSchema = new Schema({
    ...
    want: [{
        ...
        enum: [ingredients_names, "Unknown ingredient given as a want."]
    }],
    have: [{
        type: [String, "Unknown ingredient given as a have."],
        enum: [ingredients_names, "Unknown ingredient given as a have."]
    }],
    owner: {
    ...
    newRequest(owner, firstname, num_sandwiches, dist, expires, long, lat, want, have){
        ...
        return new Promise((resolve, reject) => {
            self.getRequestByOwner(owner).catch(reject).then( existing =>{
                ...
                if(!Array.isArray(want) || !Array.isArray(have)) reject(["Invalid wants/haves given."]);
                for(let i in want){
                    if(have.indexOf(have[i]) !== 0){
                ...
                if(isNaN(long) || isNaN(lat) || isNaN(expires.valueOf())) return reject(["Invalid location/expiry provided."]);
        ...
        self.getRequestByOwner(owner).catch(console.error).then(req => {
            ...
            self.request.find({
                ...
                for(let i in matches){
                    ...
                    p1 = (req.want.length === 0) ? 1 : req.want.reduce((acc, val)=>{
                        if(matches[i].have.indexOf(val) !== -1) acc++;
                        return acc;
                    })/req.want.length;
                    p2 = (matches[i].want.length === 0) ? 1 : matches[i].want.reduce((acc, val)=>{
                        if(req.have.indexOf(val) !== -1) acc++;
                        return acc;
                    })/matches[i].want.length;
                ...
                for(let i in self.socks){
                    if(self.socks[i][0] == owner){
                        self.socks[i][1].emit('matches.get', rematches);
                    }
                }
            });
        });
            ...
            self.request.find({owner, expires: {$gt: new Date()}}, null, {limit: 1}, (err, reqs)=>{
                ...
                for(let i in ingredients){
                    new_ings.push({
                        ...
                        want: reqs[0].want.indexOf(ingredients[i].name) !== -1,
                        have: reqs[0].have.indexOf(ingredients[i].name) !== -1
                    });
                }
                resolve(new_ings);
            });
