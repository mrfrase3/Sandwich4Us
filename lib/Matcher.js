...
class Matcher {
                ...
                newreq.save((err)=>{
                    if(err){
                        if(err.name !== "ValidationError"){
                            console.error(err);
                            return reject(["An Unknown issue occured, please contact an admin. 6456"]);
                        } else {
                            return reject(["Invalid values given"]);
                        }
                    }
    ...
    getRequestByOwner(search){
        ...
        return new Promise((resolve, reject) => {
            ...
            self.request.find({owner: search, expires: {$gt: new Date().toISOString()}}, null, {limit: 1}, (err, reqs)=>{
                if(err){
                    console.error(err);
                ...
                for(let i in reqs){
                    if(reqs[i].expires > Date.now()) return resolve(reqs[i]);
                }
                resolve(null);
            });
        });
    ...
    _update(req, num_sandwiches, dist, expires, long, lat, want, have){
        ...
        return new Promise((resolve, reject) => {
            ...
            for(let i = 0; i < req.want.length; i++){
                if(req.have.indexOf(req.want[i]) !== -1){
                    return reject(["You cannot both have and want and ingredient. Make up your mind. " + want[i]]);
                }
            }
            expires = new Date(Number(expires));
            ...
            self.request.find({
                ...
                for(let i = 0; i < matches.length && i < 20; i++){
                    ...
                    rematches.push({
                        ...
                        loc: matches[i].loc,
                        distance: self.distance(req.loc, matches[i].loc).toPrecision(2),
                        score: scores[matches[i].id],
                        num_sandwiches: matches[i].num_sandwiches,
                        want: matches[i].want,
                        have: matches[i].have,
                        updatedAt: matches[i].updatedAt.getTime(),
                        expires: matches[i].expires.getTime()
                    });
    ...
    getIngredients(owner, want, have){
        ...
        return new Promise((resolve, reject) => {
            ...
            self.request.find({owner, expires: {$gt: new Date().toISOString()}}, null, {limit: 1}, (err, reqs)=>{
                if(err){
                    console.error(err);
